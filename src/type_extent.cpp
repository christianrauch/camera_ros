...
get_extent(const libcamera::ControlId *id)
{
  ...
  IF(SensorTimestamp)
  IF(draft::AePrecaptureTrigger)
  IF(draft::AfTrigger)
  IF(draft::NoiseReductionMode)
  IF(draft::ColorCorrectionAberrationMode)
  IF(draft::AeState)
  IF(draft::AfState)
  IF(draft::AwbState)
  IF(draft::SensorRollingShutterSkew)
  IF(draft::LensShadingMapMode)
  IF(draft::SceneFlicker)
  IF(draft::PipelineDepth)
  IF(draft::MaxLatency)
  IF(draft::TestPatternMode)
