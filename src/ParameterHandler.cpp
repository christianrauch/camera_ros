...
ParameterHandler::declare(const libcamera::ControlInfoMap &controls)
{
  ...
  for (const auto &[id, info] : controls) {
    ...
    //     {
    //       rcl_interfaces::msg::IntegerRange range;
    //       range.from_value = max<libcamera::ControlTypeUnsigned16>(info.min());
    //       range.to_value = min<libcamera::ControlTypeUnsigned16>(info.max());
    //       descriptor.integer_range = {range};
    //     } break;
    //     case libcamera::ControlTypeUnsigned32:
    //     {
    //       rcl_interfaces::msg::IntegerRange range;
    //       range.from_value = max<libcamera::ControlTypeUnsigned32>(info.min());
    //       range.to_value = min<libcamera::ControlTypeUnsigned32>(info.max());
    //       descriptor.integer_range = {range};
    //     } break;
    // #endif
    //     default:
